...
def create_route_map(route_results, graph, edges_gdf, septa_gdf):
    ...
    # Add network edges colored by shade (background) - simplified for performance
    # Sample fewer edges to avoid serialization issues
    edges_sample = edges_gdf.sample(min(300, len(edges_gdf)))
    ...
    for idx, edge in edges_sample.iterrows():
        try:
            ...
            if shade_score < 0.2:
                color = '#ff0000'
            elif shade_score < 0.4:
                color = '#ff8800'
            elif shade_score < 0.6:
                color = '#ffff00'
            elif shade_score < 0.8:
                color = '#88ff00'
            else:
                color = '#00ff00'
